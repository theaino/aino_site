...
type Server struct {
	...
	Emailer       *misc.Emailer
	EmailTemplate *misc.Template
}

func NewServer(db *database.Connection, conf *config.Config) (*Server, error) {
	if conf.Release {
	...
	server.Emailer = misc.NewEmailer(conf)
	email_template, err := misc.LoadTemplate("templates/email/*.html")
	if err != nil {
		return nil, err
	}
	server.EmailTemplate = &email_template
